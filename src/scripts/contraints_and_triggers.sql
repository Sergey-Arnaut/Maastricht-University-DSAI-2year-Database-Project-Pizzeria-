-- src/scripts/constraints_and_triggers.sql
-- Enforce DOB, vegetarian/vegan integrity, and global single-use discount codes.
...
-- 2) Discount code single-use globally (one redemption per code)
ALTER TABLE DiscountRedemption
  ADD UNIQUE KEY idx_discount_global (discount_id);

-- 3) Vegetarian/Vegan integrity on pizza composition
...
BEGIN
  ...
  SELECT is_vegetarian, is_vegan
    INTO v_is_vegetarian, v_is_vegan
  FROM Pizza WHERE pizza_id = NEW.pizza_id;

  SELECT vegetarian, vegan
    INTO ing_veg, ing_vegan
